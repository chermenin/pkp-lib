 ...
 *}
<a href="#" id="{$buttonId|escape}" title="{$action->getHoverTitle()|escape}" class="pkp_controllers_linkAction pkp_linkaction_{$action->getId()}">
    <i aria-hidden="true" class="fa fa-{$action->getImage()}"></i>
    {if $anyhtml}{$action->getTitle()}{else}{$action->getTitle()|strip_unsafe_html}{/if}
</a>
